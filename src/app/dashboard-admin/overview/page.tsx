...
export default function Overview() {
    ...
    const fetchData = useCallback(async (roundId?: number) => {
        const round = roundId ?? selectedRound;
        if (!round) return;
        setLoading(true);
        try {
            // 1. Fetch ข้อมูลพร้อมกันทั้งหมด (Parallel)
            const [
                { data: heads },
                { data: details },
                { data: avgAssessmentData },
                { data: avgFacultyData },
                { data: facultiesData },
                { count: sCount },
                { count: tCount },
                ...
                supabase.from('avg_assessment').select('*').eq('around_id', selectedRound),
                supabase.from('avg_faculty').select('*').eq('around_id', selectedRound),
                supabase.from('faculties').select('id, faculty_name'),
                supabase.from('students').select('*', { count: 'exact', head: true }),
                supabase.from('teachers').select('*', { count: 'exact', head: true }),
                supabase.from('avg_teacher').select('teacher_id').eq('around_id', selectedRound)
            ]);
            ...
            setTotalFaculties(facultiesData ? facultiesData.length : 0);
            setParticipatedTeachers(new Set(teacherStats?.map((x: any) => x.teacher_id)).size);

            // 2. Process Structure Maps
            ...
            const validQuestions = details?.filter(d => {
                if (d.type === 'score') return true;
                if (Number.isInteger(Number(d.section2))) {
                    detailTitleMap.set(`${d.section1}-${d.section2}`, d.detail);
                }
                return false;
            }) || [];

            // 3. คำนวณแบบ Weighted Average จาก avg_assessment
            const questionStats = new Map<number, { avgScore: number; count: number }>();
            let globalWeightedSum = 0;
            let globalTotalRespondents = 0;
            let maxRespondents = 0;

            avgAssessmentData?.forEach(row => {
                const avgScore = Number(row.total_score);
                const count = Number(row.respondent_count);
                questionStats.set(row.question_id, { avgScore, count });
                globalWeightedSum += avgScore * count;
                globalTotalRespondents += count;
                if (count > maxRespondents) maxRespondents = count;
            });

            setOverallScore(globalTotalRespondents > 0 ? Number((globalWeightedSum / globalTotalRespondents).toFixed(2)) : 0);

            // 4. คำนวณระดับคณะ (Faculty Weighted Average)
            const facultyGroup = new Map<number, { sum: number; count: number }>();
            avgFacultyData?.forEach(row => {
                if (!facultyGroup.has(row.faculty_id)) facultyGroup.set(row.faculty_id, { sum: 0, count: 0 });
                const f = facultyGroup.get(row.faculty_id)!;
                f.sum += Number(row.total_score) * Number(row.respondent_count);
                f.count += Number(row.respondent_count);
            });

            const computedFacultyData: FacultyScore[] = Array.from(facultyGroup.entries()).map(([id, stats]) => ({
                name: facultyMap.get(id) || `Faculty ${id}`,
                score: stats.count > 0 ? Number((stats.sum / stats.count).toFixed(2)) : 0
            })).sort((a, b) => b.score - a.score);

            setRespondentCount(facultyGroup.size);

            // 5. คำนวณรายด้านและรายข้อ
            const domainGroup = new Map<number, { sum: number; count: number; name: string }>();
            const processedQuestions: AssessmentItem[] = validQuestions.map(d => {
                const stats = questionStats.get(d.id) || { avgScore: 0, count: 0 };
                const domainKey = Math.floor(Number(d.section2));
                const domainName = detailTitleMap.get(`${d.section1}-${domainKey}`) ||
                    domainMap.get(d.section1) || `ด้านที่ ${domainKey}`;

                if (!domainGroup.has(domainKey)) domainGroup.set(domainKey, { sum: 0, count: 0, name: domainName });
                const dg = domainGroup.get(domainKey)!;
                dg.sum += stats.avgScore * stats.count;
                dg.count += stats.count;
            ...
            const finalDomainData: AssessmentDomain[] = Array.from(domainGroup.entries()).map(([id, stats]) => {
    ...
    const handleRecalculate = async () => {
        ...
        try {
            const { error } = await supabase.rpc('calculate_assessment_summary', {
                p_around_id: selectedRound
            });
    ...
    // Initial load: fetch rounds AND data in one shot to avoid the extra re-render cycle
        ...
        const initialLoad = async () => {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    const uniqueRounds = Array.from(new Set(data.map(item => item.around_id))) as number[];
                    setRounds(uniqueRounds);
                    const firstRound = uniqueRounds[0];
                    setSelectedRound(firstRound);
                    // Fetch data immediately — no extra re-render cycle
                    await fetchData(firstRound);
                } else {
                    setLoading(false);
                }
            } catch (err) {
    ...
    // Re-fetch when user manually changes the round (skip the very first render)
    useEffect(() => {
        if (isInitialLoad.current) return; // skip — handled by initialLoad above
        if (selectedRound) {
                        ...
                        <button
                            onClick={() => {
                                if (selectedRound) {
                                    ...
                                    const url = `/dashboard-admin/overview/print?round=${selectedRound}`;
                    ...
                    <div className="bg-white/80 backdrop-blur-sm p-8 rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/50 relative overflow-hidden group hover:translate-y-[-4px] transition-all duration-500">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex items-baseline gap-3 mt-2">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xl text-slate-300 font-bold">/ {totalFaculties.toLocaleString()}</span>
                                    <span className="text-xs font-bold text-slate-400 mt-1">Total Faculties</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-8">
                                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden p-0.5">
                                    <div
                                        ...
                                        style={{ width: `${(respondentCount / (totalFaculties || 1)) * 100}%` }}
                                    />
                                </div>
                            </div>
                        </div>
